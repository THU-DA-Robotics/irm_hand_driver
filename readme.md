...
由于使用的FT电机与Leap Hand电机尺寸、转轴、固定孔位置类似，我们可以大致使用教程https://v1.leaphand.com/assembly 进行灵巧手硬件的安装。请注意：我们的S101有两个版本，分别是需要8片的普通版和2片的Slim版。Slim版用于侧面不能突出的大拇指第二关节部分。除此之外，在2、3、4指的非屈伸关节多了一个打印的圆柱垫片，具体如“组装”一节所示。
...
- 电机有安装方向之分，但不管装正装反都不影响灵巧手本质结构，可以用电机Logo及型号两面的朝向分辨其安装方向。我们组装的灵巧手各电机方向如下图（2、3、4指朝向相同），如果你组装的手和我们的电机朝向相同，那么在完成后续“电机设置”一节完成后你应该得到正方向与本文描述完全相同的结果。在这一步将电机方向装反问题不大，只需在电机设置一节确保各关节正方向与本文描述相同即可。
- **由于更改了S102的螺丝孔结构，与LEAP结构不同的是，需要在2、3、4指由手掌起第二电机与第三电机连接处（唯一的非屈伸关节）需要在第二电机舵盘与第三电机的S101之间加入一圆形垫片**`圆柱垫块.SLDPRT/STL`，否则容易由于摩擦损坏电机。
...
- **我们强烈建议你参照如下电机正方向（向该方向旋转角度增加）检查一遍**：对于大拇指来说，正方向由手掌至指尖为：掌心、逆时针、掌心、掌心。其余三指由手掌至指尖：掌心、左、掌心、掌心。
